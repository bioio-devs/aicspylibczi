...
`/etc/ssl` or `/etc/pki`. Set `SSL_CERT_FILE` (or `CURL_CA_BUNDLE`) or pass the `ca_info`
stream option to use a different bundle. macOS and Windows use the OS certificate store.
